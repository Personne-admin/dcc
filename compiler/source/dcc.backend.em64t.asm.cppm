...
namespace
{
    ...
    [[nodiscard]] std::string emit_globals(ir::IrModule const& ir_mod, std::unordered_set<std::string>& defined_syms, dcc::target::TargetConfig const& target)
    {
        ...
        {
            if (!func || func->name.empty() || !func->blocks.empty())
                continue;
            if (target.object_format == dcc::target::ObjectFormat::Coff && func->is_dll_import)
                externs.push_back("__imp_" + std::string{func->name});
            else
                externs.push_back(std::string{func->name});
        }
        for (auto* g : ir_mod.globals)
        {
            if (!g)
...
export namespace dcc::backend::em64t
{
    [[nodiscard]] std::string emit_intel_asm(ir::IrModule const& ir_mod, std::vector<MFunction> const& functions, target::TargetConfig const& target)
    {
        ...
        out += emit_globals(ir_mod, defined_syms, target);
        return out;
    }
